...
<!--chapter:end:25-functions-and-for-loops.Rmd-->
...
You can get free png silhouettes of various animals from [phylopic.org](http://phylopic.org).You can use the function `image_data()` to call the image's universally unique identifier (UUID) and specify the size of the image.
...
Before beginning, ensure you have installed both `ggplot2` and `rphyolopic`. Silhouettes can be added to a ggplot graphic by using the function [add_phylopic()](https://www.rdocumentation.org/packages/rphylopic/versions/0.3.0/topics/add_phylopic) as another layer.
...
ggplot() + 
  geom_point(data = data, aes(x = x, y = y)) + 
  add_phylopic(squid.img, alpha = 0.2, x = 7, y = 4, ysize = 1, color = "blue")
```
...
<!--chapter:end:27-advanced_ggplot_silhouettes.Rmd-->
...
<!--chapter:end:28-making-maps.Rmd-->
