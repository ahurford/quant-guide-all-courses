...
melt(data2, id.var = c('person', 'trial'), variable.name = 'outcomes')

```

3. Another way to reshape data between these formats is to use the following two functions from the `tidyr` package in R:
...
Again, to use the functions, we would be required to run `install.packages("tidyr")` to get the package installed if you have not done that previously. Also remember to load the package. 
